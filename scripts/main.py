...
if __name__ == "__main__":
    ...
    logger_main = (
        TimeLogger(args, 1, hierachy=5, model_name=args.results_path)
        if args.time_logger_verbose >= 1
        else TimeLogger(args, 0, model_name=args.results_path)
    )
    ...
    print(
        "Number of patient for -Train:{},-Dev:{}, -Test:{}, --Attr:{}".format(
            train_data.__len__(),
            dev_data.__len__(),
            test_data.__len__(),
            attribution_set.__len__(),
        )
    )
    ...
    if args.train:
        ...
        pickle.dump(args_dict, open(results_path, "wb"))
        pickle.dump(
            epoch_stats, open("{}.{}".format(args.results_path, "epoch_stats"), "wb")
        )
        del epoch_stats
    ...
    if args.dev:
        ...
        pickle.dump(args_dict, open(results_path, "wb"))
        pickle.dump(
            dev_stats, open("{}.{}".format(args.results_path, "dev_stats"), "wb")
        )
        pickle.dump(
            dev_preds, open("{}.{}".format(args.results_path, "dev_preds"), "wb")
        )
    ...
    if args.test:
        ...
        pickle.dump(
            test_stats, open("{}.{}".format(args.results_path, "test_stats"), "wb")
        )
        pickle.dump(
            test_preds, open("{}.{}".format(args.results_path, "test_preds"), "wb")
        )
    ...
    if args.attribute:
        ...
        test_attribution, test_censored_attribution = attribute.compute_attribution(
            attribution_set, model_for_attribution, args
        )
        print("Save attribution results to {}".format(args.results_path))
        args_dict = vars(args).copy()
        del args_dict["code_to_index_map"]
        pickle.dump(args_dict, open(args.results_path, "wb"))
        logger_main.log("ATTRIBUTION")

        pickle.dump(
            test_attribution,
            open("{}.{}".format(args.results_path, "test_attribution"), "wb"),
        )
        pickle.dump(
            test_censored_attribution,
            open("{}.{}".format(args.results_path, "test_censored_attribution"), "wb"),
        )
        logger_main.log("Dump results")
