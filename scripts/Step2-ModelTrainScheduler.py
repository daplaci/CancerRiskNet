...
parser.add_argument(
    ...
    type=str,
    default="searches",
    help="The location to store search summary.",
)
parser.add_argument(
    "--scheduler",
    type=str,
    default="single_node_scheduler",
...
parser.add_argument(
    "--machines",
    type=str,
    default="configs/avai_machines.txt",
...
def single_node_scheduler(workers):
    assert (
        len(workers) == 1 and args.n_workers == 1
    ), "n_workers does not equal to one. Cannot use single node worker."
    worker = workers[0]
...
def gcp_scheduler(workers):
    ...
    if args.n_workers != len(machines):
        warnings.warn(
            "[W] The input n_workers ({}) does not match the available machines ({}).".format(
    ...
    for worker, hostname, machine in zip(workers, hostnames, machines):
        ...
        print(
            "[INFO] Worker {} is launched on machine {} ({})".format(
                worker, hostname, machine
            )
        )
...
def gcp_worker(args, worker_id, machine):
    ...
    os.system(
        "ssh {} '{}'".format(
            machine, "cd ~; git checkout add_code_to_index_map; git pull;"
        )
    )
    ...
    os.system(
        "nohup ssh {} 'cd ~/ ; {}; {}; sudo shutdown' &".format(machine, cmd1, cmd2)
    )
...
def torque_scheduler(workers):
    for worker in workers:
        ...
        shell_cmd = [
            ...
            "#PBS -e {}/.$PBS_JOBID.err",
            "#PBS -o {}/.$PBS_JOBID.out",
            "python scripts/schedulers/dispatcher.py {}",
        ]

        shell_cmd = "\n".join(shell_cmd).format(
            args.search_summary_dir, args.search_summary_dir, flag_string
        )
...
if __name__ == "__main__":
    ...
    grid_logs_path_regex = os.path.join(
    ...
    print(
        COMMAND_TO_COLLECT_SEARCH.format(
            args.experiment_config_path, args.search_summary_dir, args.save_dir
        )
    )
    sys.exit(0)
