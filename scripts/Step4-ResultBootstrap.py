...
def get_boot_metric_clf(n):
    if n > 0:
        sample = np.random.choice(
            probs_for_eval.size, probs_for_eval.size, replace=True
        )
    ...
    fps, tps, thresholds = _binary_clf_curve(golds, probs, pos_label=1)
    ...
    if n == 0:
        return {
            "precision": precisions[::20].tolist(),
...
def get_performance_ci(
    ...
    auroc, fpr, tpr, auprc, precision, recall, odds_ratio, incidence, threshold = zip(
        *metrics
    )
    incidence_ci = mean_confidence_interval(
        incidence,
        model_name,
        "incidence",
        prediction_interval,
    ...
    auroc_ci = mean_confidence_interval(
        auroc, model_name, "auroc", prediction_interval, exclusion_interval, exp_id
    )
    fpr_ci = mean_confidence_interval(
        fpr, model_name, "fpr", prediction_interval, exclusion_interval, exp_id
    )
    tpr_ci = mean_confidence_interval(
        tpr, model_name, "tpr", prediction_interval, exclusion_interval, exp_id
    )
    auprc_ci = mean_confidence_interval(
        auprc, model_name, "auprc", prediction_interval, exclusion_interval, exp_id
    )
    precision_ci = mean_confidence_interval(
        precision,
        model_name,
        "precision",
        prediction_interval,
    ...
    recall_ci = mean_confidence_interval(
        recall, model_name, "recall", prediction_interval, exclusion_interval, exp_id
    )
    odds_ratio_ci = mean_confidence_interval(
        odds_ratio,
        model_name,
        "odds_ratio",
        prediction_interval,
    ...
    threshold_ci = mean_confidence_interval(
        threshold,
        model_name,
        "thershold",
        prediction_interval,
...
if __name__ == "__main__":
    ...
    for i, (exp_id, save_dir, model_name, exclusion_interval) in enumerate(
        zip(
            best_exp_ids_config["exp_id"],
            best_exp_ids_config["save_dir"],
            best_exp_ids_config["model_name"],
            best_exp_ids_config["exclusion_interval"],
        )
    ):
        printing_prefix = "[Step5-ResultsBootstrap][{}/{}][{}]".format(
            i + 1, len(best_exp_ids_config["exp_id"]), exp_id
        )
        ...
        for index, month in enumerate(results["month_endpoints"]):
            ...
            experiment_performance = get_performance_ci(
                probs_for_eval,
                golds_for_eval,
                model_name,
                month,
    ...
    os.makedirs(
        os.path.join(os.path.dirname(args.search_metadata), "figures"), exist_ok=True
    )
    ...
    get_slice(df, metric_name="auroc").pivot_table(
        ...
        index=["Model", "Exclusion Interval"],
    ...
    get_slice(df, metric_name=["fpr", "precision", "recall"]).pivot_table(
        values="print_merged",
        index=["Model", "Exclusion Interval", "Metric"],
        columns=["Prediction Interval"],
