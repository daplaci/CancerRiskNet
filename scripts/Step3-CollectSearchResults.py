...
CONFIG_NOT_FOUND_MSG = (
    "[Step3-CollectSearchResults][1/3][INFO] {}: {} config file not specified. Summarizing "
...
parser.add_argument(
    ...
    type=str,
    default="results",
    help="Where to store logs and result files.",
)
parser.add_argument(
    "--search_dir",
    type=str,
    default="searches",
    help="Where to store the information for the search.",
)
parser.add_argument(
    "--metric",
    type=str,
    default="dev_{}month_auprc",
    ...
    try:
        ...
        for i in ["dev", "test"]:
            if os.path.exists("{}.{}_stats".format(result_path, i)):
                ...
                print(
                    "[Step3-CollectSearchResults][2/3][WARNING] Missing {} stats.".format(
                        i
                    )
                )
    except FileNotFoundError:
        print(
            "[Step3-CollectSearchResults][2/3][ERROR] Experiment failed or the result file is in another location!"
            " Logs are located at: {}".format(log_path)
        )
        return summary, None
...
def print_grid_performances(summary_df, args_df_subset):
    ...
    if "exclusion_interval" in args_df_subset.columns:
        ...
        exclusion_indexes.extend(
            [args_df_subset.exclusion_interval == ei for ei in unique_exclusions]
        )
    ...
    for einterval, eindex in zip(exclusion_interval, exclusion_indexes):
        ...
        summary_w_all_args = (
            summary_df[["score"]]
            .merge(args_df_exclusion, left_index=True, right_index=True)
            .sort_values(by="score", ascending=False)
        )

        for use_known_risk, summary_by_risk_factor in summary_w_all_args.groupby(
            "use_known_risk_factors_only"
        ):
            for model, summary_by_model in summary_by_risk_factor.groupby("model_name"):
                if type(einterval) is not int or (
                    type(einterval) is str and not einterval.isdigit()
                ):
                    continue
        ...
        args_df_exclusion = args_df_exclusion.loc[
            :, args_df_exclusion.astype("str").nunique() > 1
        ]
        summary_w_changing_args = (
            summary_df[["score"]]
            .merge(args_df_exclusion, left_index=True, right_index=True)
            .sort_values(by="score", ascending=False)
        )
        ...
        t_args = [
            arg
            for arg in t_args
            if arg not in ["exp_id", "model_name", "results_path", "save_dir"]
        ]
        ...
        for aggregate, combo in enumerate(combos):
            ...
            figwidth = (
                leftmargin + rightmargin + ((len(combo[0][0]) + 1) ** 2) * categorysize
            )
            ...
            for cols in combo:
                ...
                if len(cols) == 1:
                    ...
                    param = "-".join(cols)
                    summary_w_changing_args[param] = (
                        summary_w_changing_args.loc[:, cols]
                        .astype(str)
                        .agg("-".join, axis=1)
                    )
                sns.swarmplot(
                    ...
                    y="score",
                    ax=ax,
                    data=summary_w_changing_args.sort_values(by=param),
                )
                # add line to connect similar experiments
                unique_vals = sorted(summary_w_changing_args[param].unique())
                grouped_duplicates = summary_w_changing_args[
                    [c for c in t_args if c not in cols]
                ]
                grouped_duplicates = (
                    grouped_duplicates.groupby(grouped_duplicates.columns.tolist())
                    .apply(lambda x: tuple(x.index))
                    .reset_index(name="idx")
                )
                paired = [
                    pair for pair in grouped_duplicates.idx.tolist() if len(pair) > 1
                ]
                for pair in paired:
                    ...
                    for exp_id in pair:
                        ...
                        scores[
                            unique_vals.index(current_param)
                        ] = summary_w_changing_args.loc[exp_id, "score"]
            ...
            plt.savefig(
                figure_path
                + "/grid.aggregate{}_exclusion_{}.png".format(aggregate, einterval),
                bbox_inches="tight",
            )

    best_exp_summary = pd.DataFrame.from_records(
        best_exp_summary,
...
if __name__ == "__main__":
    ...
    print(
        SUMMARIZING_MSG.format(
            len(job_list), args.search_dir + "/master.{}.summary".format(master_id)
        )
    )

    with open(
        args.search_dir + "/master.{}.joblist".format(master_id), "w"
    ) as out_file:
    ...
    if not args.skip_loading:  # TODO: why do we want to skip loading?
        ...
        for job in job_ids:
            ...
            summary, job_args = update_summary_with_results(
                result_path, log_path, summary, summary_path
            )
            if not job_args:
                continue
    ...
    summary_df.index = [
        os.path.basename(log_path).split(".")[0] for log_path in summary_df.log_path
    ]
    ...
    summary_df_test_only = summary_df.loc[
        :, [k for k in summary_df.keys() if "test" in k and "roc" in k]
    ]
    merge_df = args_df.merge(summary_df_test_only, left_index=True, right_index=True)
    merge_df_brief = args_vis.merge(
        summary_df_test_only, left_index=True, right_index=True
    )
    merge_df.to_csv(os.path.join(figure_path, "master.merge_info.{}.csv".format(exp)))
    merge_df_brief.to_csv(
        os.path.join(figure_path, "master.merge_info_brief.{}.csv".format(exp))
    )
    ...
    vis_df(
        ...
        (args_df != args_df.iloc[0]).any(),
    )
    vis_df(
        summary_df,
        summary_txt,
        "C-Index (all 3 datasets)",
        [p + "_c_index" for p in partitions],
    )
    vis_df(
        summary_df,
        summary_txt,
        "AUROC - (Dev) Full eval",
        ["dev_{}_auroc".format(s) for s in time_series],
    )
    vis_df(
        summary_df,
        summary_txt,
        "AUPRC - (Dev) Full eval",
        ["dev_{}_auprc".format(s) for s in time_series],
    )

    print(
        NEXT_COMMAND.format(os.path.join(args.search_dir, "performance_table.csv"))
    )  # TODO performance_table.csv name could also be an argument in parser
