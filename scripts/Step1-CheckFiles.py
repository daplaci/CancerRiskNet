...
assert (
    sys.version >= "3.7"
), "[Step1-CheckFiles][1/3] Python version not compatible. v3.7 is required. Abort."

print("[Step1-CheckFiles][1/3] Checking primary location...")
assert (
    "scripts/Step1-CheckFiles.py" not in os.getcwd()
...
try:
    ...
    print(
        "[Step1-CheckFiles][1/3] CancerRiskNet package cannot be found. Check your environment and then continue."
    )
...
settings = []
setting_paths = (
    grid_search_config["search_space"]["data_setting_path"]
    if "data_setting_path" in grid_search_config["search_space"]
    else ["data/settings_RPDR.yaml"]
)
...
for k, path in enumerate(setting_paths):
    ...
    try:
        ...
        for key in ["PANC_CANCER_CODE", "END_OF_TIME_DATE"]:
            assert (
                key in setting
            ), "[Step1-CheckFiles][3/3]{} An input for {} is required in {}. Aborting.".format(
                idx, key, path
            )
            print(
                "[Step1-CheckFiles][2/3]{} Item {} is found in {}.".format(
                    idx, key, path
                )
            )
        ...
        for key in ["ICD8_MAPPER_NAME", "ICD9_MAPPER_NAME", "ICD10_MAPPER_NAME"]:
            if key not in setting:
                warnings.warn(
                    "[Step1-CheckFiles][3/3]{} Warning. Mapper file {} not found.".format(
                        idx, key
                    )
                )
            ...
            assert os.path.exists(
                map_path
            ), "[Step1-CheckFiles][3/3]{} Mapper file {} not found. Aborting.".format(
                idx, map_path
            )
            print(
                "[Step1-CheckFiles][2/3]{} Mapper file {}->{} found.".format(
                    idx, key, map_path
                )
            )
...
for k, metadata_path in enumerate(metadata_paths):
    ...
    try:
        ...
        print(
            "[Step1-CheckFiles][3/3]{} Metadata {} not found. Aborting.".format(
                idx, metadata_path
            )
        )
        sys.exit(1)
    ...
    vocab_path = os.path.join(
        ...
        os.path.basename(metadata_path).replace(".json", "-vocab.txt"),
    )
    if os.path.exists(vocab_path):
        print(
            "[Step1-CheckFiles][3/3]{} The vocabulary for metadata {} found! Checking...".format(
                idx, metadata_path
            )
        )
        ...
        print(
            "[Step1-CheckFiles][3/3]{} For metadata {}, no vocabulary is detected. Automatically generating...".format(
                idx, metadata_path
            )
        )
        codes = set()
    ...
    print(
        "[Step1-CheckFiles][3/3]{} For vocabulary {} checking inclusion...".format(
            idx, vocab_path
        )
    )
    assert all(
        [known_factors not in codes for known_factors in known_factor_lists]
    ), "[Step1-CheckFiles][3/3]{} For metadata {}, no known_factors are in the vocabulary. Abort."

    cancer_codes = cancer_code_lists[k]
    not_found_codes = [
        cancer_code for cancer_code in cancer_codes if cancer_code not in codes
    ]
    assert len(not_found_codes) != len(
        cancer_codes
    ...
    if not_found_codes:
        warnings.warn(
            "[Step1-CheckFiles][3/3]{} Warning. For metadata {}, one of more PANC_CANCER_CODE not found in vocabulary. "
...
print(
    "[Step1-CheckFiles][NEXT] Proceed to step 2, e.g., do `python scripts/Step2-ModelTrainScheduler.py "
