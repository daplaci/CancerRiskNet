...
parser.add_argument(
    ...
    required=True,
    type=str,
    help="Path of sub-experiment config file.",
)
parser.add_argument(
    "--summary_path",
    type=str,
...
def update_summary_with_results(results_path, log_path, result_keys):
    ...
    try:
        ...
        return None
    try:
        epoch_stats = pickle.load(
            open("{}.{}".format(results_path, "epoch_stats"), "rb")
        )
    except FileNotFoundError:
...
if __name__ == "__main__":
    ...
    multiprocessing.Process(
        target=queue_worker, args=(args.gpu, job_queue, done_queue)
    ).start()
