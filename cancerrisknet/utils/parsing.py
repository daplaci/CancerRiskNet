...
POSS_VAL_NOT_LIST = (
    "Flag {} has an invalid list of values: {}. Length of slist must be >=1"
)
...
def parse_args(args_str=None):
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="num workers for each data loader [default: 8]",
    )
    ...
    parser.add_argument(
        ...
        help="Path of json source datafile",
    )
    parser.add_argument(
        "--data_setting_path",
        type=str,
        default="data/settings.yaml",
        help="Path of yaml with data specific settings",
    )
    parser.add_argument(
        "--month_endpoints",
        nargs="+",
        default=[3, 6, 12, 36, 60, 120],
    ...
    parser.add_argument(
        "--no_random_sample_eval_trajectories",
        action="store_true",
        default=False,
    ...
    parser.add_argument(
        "--model_name", type=str, default="transformer", help="Model to be used."
    )
    parser.add_argument(
        "--num_layers",
        type=int,
    ...
    parser.add_argument(
        "--save_dir", type=str, required=True, help="The output file location."
    )
    parser.add_argument(
        "--model_dir",
        type=str,
        default="snapshots",
        help="The path to the library of trained models.",
    )
    parser.add_argument(
    ...
    args = (
        parser.parse_args() if args_str is None else parser.parse_args(args_str.split())
    )
    ...
    if args.train:
        assert args.dev, Exception(
            "[E] --dev is disabled. The dev dataset is required if --train for turning purpose."
        )
    ...
    if args.test or args.dev:
        if (
            ...
            and args.tuning_metric is not "loss"
        ):
            raise Exception(
        ...
        assert any(
            [args.eval_auroc, args.eval_auprc, args.eval_mcc, args.eval_c_index]
        ), Exception(
    ...
    if args.num_heads is not None and args.model_name != "transformer":
        ...
        args.num_heads is None
        and args.model_name == "transformer"
        and not args.resume_from_result
    ):
        raise Exception(
            "[E] The `num_heads` is required for transformer models. Add that to your config and try again."
        )
...
def load_data_settings(args):
    ...
    SETTINGS.chapterColors = [
        (r / 255, g / 255, b / 255) for r, g, b, in SETTINGS.chapterColors
    ]

    CODEDF = pd.read_csv(
        SETTINGS.ICD10_MAPPER_NAME,
        sep="\t",
        header=None,
        names=["code", "description", "chapter", "chapter_name", "block_name", "block"],
    )
    CODEDF = CODEDF.append(
        pd.read_csv(
            SETTINGS.ICD8_MAPPER_NAME,
            sep="\t",
            header=None,
            names=[
                "code",
    ...
    return {
        "CODE2DESCRIPTION": CODE2DESCRIPTION,
        "SETTINGS": SETTINGS,
        "CODEDF_w_9": CODEDF_w_9,
        "CODEDF": CODEDF,
    }
...
def get_code(args, event, char=False):
    ...
    if code[0] == "D" and not code[1].isdigit():  # this means it is a SKS code
        return code[
            : args.icd10_level + 1
        ]  # TODO data - check replacement before truncation or after?
    elif code.isdigit():
        return code[: args.icd8_level]
    elif (
        code[0] == "Y" or code[0] == "E"
    ):  # TODO data - separate SKS or RPDR code by the data class not by filtering
