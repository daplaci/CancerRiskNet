...
def save_figure_and_subplots(figname, fig, **kwargs):
    ...
    for ax_num, ax in enumerate(fig.axes):
        extent = ax.get_tightbbox(renderer=fig.canvas.get_renderer()).transformed(
            fig.dpi_scale_trans.inverted()
        )
        fig.savefig(
            "{}_{}.{}".format(figname, ax_num, kwargs["format"]),
            bbox_inches=extent,
            **kwargs
        )
    fig.savefig(
        "{}.{}".format(figname, kwargs["format"]), bbox_inches="tight", **kwargs
    )
