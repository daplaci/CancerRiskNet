...
class DiseaseProgressionDataset(data.Dataset):
    def __init__(self, metadata, args, split_group):
        ...
        for patient in tqdm.tqdm(metadata):
            ...
            if (
                split_group != "all"
                and patient_metadata[patient]["split_group"] != split_group
            ):
                continue
            ...
            future_panc_cancer, outcome_date = self.get_outcome_date(
                events, end_of_date=obs_time_end
            )
            patient_dict.update(
                {
                    "future_panc_cancer": future_panc_cancer,
                    "dob": dob,
            ...
            patient_dict.update(
                {"avai_indices": avai_indices, "y": gold, "events": events}
            )
        ...
        print(
            "Number of positive patients  in '{}' dataset is: {}.".format(
                self.split_group, total_positive
            )
        )
        self.class_count()
    ...
    def process_events(self, events):
        ...
        if self.args.use_known_risk_factors_only:
            for e in events:
                if (
                    e["codes"] not in self.SETTINGS.KNOWN_RISK_FACTORS
                    and e["codes"] not in self.SETTINGS.PANC_CANCER_CODE
                ):
                    e["codes"] = PAD_TOKEN
        return events
    ...
    def get_trajectory(self, patient):
        ...
        if self.split_group in ["dev", "test", "attribute"]:
            if not self.args.no_random_sample_eval_trajectories:
                selected_idx = [
                    random.choice(patient["avai_indices"])
                    for _ in range(self.args.max_eval_indices)
                ]
            else:
    ...
    def get_time_seq(self, events, reference_date):
        ...
        """
        deltas = np.array(
            [abs((reference_date - event["admit_date"]).days) for event in events]
        )
        multipliers = (
            2
            * np.pi
            / (
                np.linspace(
                    ...
                    stop=MAX_TIME_EMBED_PERIOD_IN_DAYS,
                    num=self.args.time_embed_dim,
                )
            )
        )

        deltas, multipliers = deltas.reshape(len(deltas), 1), multipliers.reshape(
            1, len(multipliers)
        )
    ...
    def get_label(self, patient, until_idx):
        ...
        num_time_steps, max_time = len(self.args.month_endpoints), max(
            self.args.month_endpoints
        )
    ...
    def get_outcome_date(self, events, end_of_date=None):
        ...
        panc_ca_events = [
            e
            for e in events
            if any(icd == e["codes"] for icd in self.SETTINGS.PANC_CANCER_CODE)
        ]
    ...
    def __getitem__(self, index):
        ...
        for sample in samples:
            ...
            x = [
                self.get_index_for_code(code, self.args.code_to_index_map)
            ...
            item = {
                ...
                "time_seq": pad_arr(
                    time_seq, self.args.pad_size, np.zeros(self.args.time_embed_dim)
                ),
                "age_seq": pad_arr(
                    age_seq, self.args.pad_size, np.zeros(self.args.time_embed_dim)
                ),
                "code_str": code_str,
            }
            for key in [
