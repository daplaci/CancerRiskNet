...
class StateKeeper:
    ...
    def save(self, models, optimizers, epoch, lr, epoch_stats):
        ...
        param_path = os.path.join(
            self.args.model_dir, PARAM_PATH.format(self.args.model_name, identifier)
        )
        ...
        stats_path = os.path.join(
            self.args.model_dir, STATS_PATH.format(self.args.model_name, identifier)
        )
        ...
        for model_name in models:
            ...
            model_path = os.path.join(
                self.args.model_dir, MODEL_PATH.format(model_name, identifier)
            )
            ...
            optimizer_path = os.path.join(
                self.args.model_dir, OPTIMIZER_PATH.format(model_name, identifier)
            )
        ...
        for model_name in model_names:
            ...
            try:
                models[model_name] = torch.load(
                    model_path, map_location=self.args.device
                )
            except Exception:
            ...
            try:
                optimizer_states[model_name] = torch.load(
                    optimizer_path, map_location=self.args.device
                )
            except Exception:
            ...
            for param_key in optimizer.state_dict()["state"]:
                ...
                for attribute_key in param:
                    if isinstance(param[attribute_key], int) or isinstance(
                        ...
                        continue
                    optimizer.state_dict()["state"][param_key][
                        attribute_key
