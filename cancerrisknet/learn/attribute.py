...
from cancerrisknet.utils.learn import (
...
def compute_attribution(attribute_data, model, args):
    ...
    if hasattr(model.model, "a_embed_add_fc") and hasattr(
        model.model, "a_embed_scale_fc"
    ):
        age_embeddings_layers = [
            model.model.a_embed_add_fc,
            model.model.a_embed_scale_fc,
        ]
    ...
    try:
        for i, batch in enumerate(tqdm(test_iterator)):
            ...
            for patient_codes, patient_attr, gold, days in zip(
                codes, attr, batch["y"], batch["days_to_censor"]
            ):
            ...
            for patient_age, patient_age_attr in zip(ages, combined_add_ages):
                word2attr["Combined-Age-{}".format(patient_age)].append(
                    patient_age_attr
                )
...
def attribute_batch(explain_code, explain_age, batch, month_idx=3):
    ...
    if explain_code:
        attributions_code = explain_code.attribute(
            inputs=(batch["x"], batch["age_seq"], batch["time_seq"]),
            n_steps=2,
    ...
    if explain_age:
        attributions_age = explain_age.attribute(
            inputs=(batch_age["x"], batch_age["age_seq"], batch_age["time_seq"]),
            n_steps=2,
            return_convergence_delta=False,
            target=month_idx,
        ...
        age_attribution_combined = (
            (attributions_age[0] + attributions_age[1]).cpu().detach().numpy()
        )
    else:
        age_attribution_add = []
    ...
    return (
        batch["code_str"],
        attributions_code,
        (batch_age["age"] // 365).squeeze().tolist(),
        age_attribution_add,
        age_attribution_scale,
        age_attribution_combined,
    )
