...
def train_model(train_data, dev_data, model, args):
    ...
    logger_epoch = (
        TimeLogger(args, 1, hierachy=2)
        if args.time_logger_verbose >= 2
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        best_func, arg_best = (
            (min, np.argmin) if "loss" in tuning_key else (max, np.argmax)
        )
...
def eval_model(eval_data, name, models, args):
    ...
    (
        ...
        train=False,
        truncate_epoch=(
            not args.exhaust_dataloader and eval_data.split_group != "test"
        ),
        models=models,
        optimizers=None,
        args=args,
