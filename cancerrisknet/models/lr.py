...
class OneHotRiskModel(AbstractRiskModel):
    ...
    def __init__(self, args):
        ...
        num_features_in = (
            self.vocab_size + 1 if args.add_age_neuron else self.vocab_size
        )
...
class ProportionalHazards(OneHotRiskModel):
    ...
    def __init__(self, args):
        assert (
            args.pool_name != "Softmax_AttentionPool"
        ), "COX models are not compatible with attention poolings"
        super(ProportionalHazards, self).__init__(args)
        self.add_module(
            "linear_layer_hz", nn.Linear(1, len(self.args.month_endpoints), bias=False)
        )
        self.exp = torch.exp
